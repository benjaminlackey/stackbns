...
def radius_confidence(mass, eoschain, conf=[sigma1, sigma2, sigma3], method='points', dx=0.1, smooth=False, showhist=False):
    """
...
def lambda_confidence(mass, eoschain, conf=[sigma1, sigma2, sigma3], method='points', dx=0.1, smooth=False, showhist=False):
    """
...
def confidence_of_mass(quantity, massmin, massmax, nmass, eoschain, conf=[sigma1, sigma2, sigma3], method='points', dx=0.1, smooth=True, showhist=False):
    """
    ...
    if quantity == 'radius':
        ...
        rofm = partial(radius_confidence, eoschain=eoschain, conf=conf, method=method, dx=dx, smooth=smooth, showhist=showhist)
        minmax = pool.map(rofm, marray)
    elif quantity == 'lambda':
        lofm = partial(lambda_confidence, eoschain=eoschain, conf=conf, method=method, dx=dx, smooth=smooth, showhist=showhist)
