...
if __name__ == "__main__":
    ...
    parser.add_argument("fisherestimation", help="Mathematica .m file that contains the {true vector, best vector, covariance matrix} for {ln(M_chirp), ln(eta), \tilde\Lambda}.")
    # Name of output file
    ...
    truelist, bestlist, covlist = binary.ReadTrueBestCovFile(args.fisherestimation)
    ...
    bestFitArray = bestlist[systemsList]
    covArray = covlist[systemsList]

    # Run the MCMC simulation
    chain = eos4p.RunMCMCFor4ParamEOS(trueArray, bestFitArray, covArray, nWalkers, nBurn, nSteps, threads=args.threads)
...
#
## Get list of true parameters, best estimate of parameters, and covariance matrix of parameters from Mathematica file
#truelist, bestlist, covlist = binary.ReadTrueBestCovFile('truebestcovsnr8m12to16fISCOmpa1fit.m')
#
## State which systems you want to use
#nSystems = 1
#systemsList = range(0, nSystems)
...
#nSteps = 10000
#
## Fetch Fisher matrix "simulation" data
#trueArray = truelist[systemsList]
#bestFitArray = bestlist[systemsList]
#covArray = covlist[systemsList]
## Run the MCMC simulation
#chain4p1bns = eos4p.RunMCMCFor4ParamEOS(trueArray, bestFitArray, covArray, nWalkers, nBurn, nSteps)
#
#
#np.save('test.npy', chain4p1bns[:, :, [0, 1, 2, 3]])
